
### Purpose of this document
To have an easy-to-look checklist for the main corrections of the paper.

### Here I talk about code stuff

- [x] Major 1: Add EB occurrence rate of 1%
- [x] Major 2: Add distribution of transit depths and durations

### Here I talk about paper/latex stuff

- [ ] Major 1: State in the latex document that is not our aim to estimate a realistic EB rate for PLATO. Such thing has be done already (see Bray et al. 2023). I can also mention the Kepler and TESS papers from Prsa. 
- [ ] Major 2: Change the results section to include the 2 scenarios: variable vs fix transit parameters
	- [ ] Write section 7.3: Scenario B: variable transit parameters
	- [ ] Write section 7.4: Comparison between scenarios
	- [ ] Show the two scenarios about EBs: the fixed values and the distribution values
	- [ ] Show the results about the value distribution of transit depths and durations
		- [ ] Efficiency plot
		- [ ] Efficiency table
- [ ] Major 3: Fix validation language. 
	- [ ] Investigate and or contact Juan for guidance about the subtle details 
	- [x] Section 2.3
	- [ ] Introduction
	- [ ] Section 6

Change the following plots and the corresponding figure captions:
- [x] Nominal mask
- [x] Secondary mask
- [x] Extended mask
- [ ] Efficiency plot for the double-aperture photometry (baseline scenario)
- [ ] Efficiency plot for the centroid shift (baseline scenario)
- [ ] Efficiency plot for the double-aperture photometry (distribution of transit depth and durations)
- [ ] Efficiency plot for the centroid shift (distribution of transit depths and durations)
 