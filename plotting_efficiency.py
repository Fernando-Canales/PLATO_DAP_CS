...
# N_bad_ext / N_bad. The thing to do is therefore to write an expression for obtaining each of these three quantities.
# In our code, we are assuming (and that is quite a strong assumption!) that all contaminant stars for every target are
# eclipsing binaries with the same transit depth for every of them. In such a case, the total number of false positives
# detectable for the nominal mask is just the number of contaminant stars that could generate a detectable signal on the
# nominal mask (i.e \eta_t > 7.1). On the other hand, the number of false positives that can be spotted by the secondary
# or Extended masks is the number of stars that can produce a detectable signal on the nominal mask as well
# (eta_t > 7.1) given that they are detectable on their respective masks (eta_c > 7.1 and eta_ext > 7.1) and
# (delta_obs_c > delta_obs_t delta_obs_ext > delta_obs_t). (This can be discussed as well)
