...
#5: spr_tot_secondary_mask (sprK_sec)
#6: eta_c
...
#24-33: eta_ext_10first_24_cameras
#34-43: eta_ext_10first_6_cameras
#44: NSR_ext_1h_6_cameras
#45-54: eta_cob_ext_10first_24_ccameras
...
td_sec = 4         # transit duration in hours
...
sigma_cob_nom_10first_6_cameras = data[:, 56:66]
...
eta_cob_ext_10first_24_cameras = np.zeros((n, 10))
eta_cob_ext_10first_6_cameras = np.zeros((n, 10))
eta_cob_nom_10first_24_cameras = np.zeros((n, 10))
eta_cob_nom_10first_6_cameras = np.zeros((n, 10))
eta_cob_sec_24_cameras = data_sec[:, 8]*np.sqrt(td_sec*ntr)/data_sec[:, 10]
eta_cob_sec_6_cameras = data_sec[:, 13]*np.sqrt(td_sec*ntr)/data_sec[:, 14]
eta_cob = data[:, 14] *np.sqrt(td_sec*ntr)/ data[:,16]
eta_cob_6_cameras = data[:, 42] *np.sqrt(td_sec*ntr)/ data[:, 43]
...
for i in range(n):
    ...
    ##dback[:] = 85000
    ##td[:] = 4.
    Lambda_nom = dback*1e-6/(1. - dback*1e-6*data[i, 17:27])
    eta_cob_nom_10first_24_cameras[i, :] = Lambda_nom * data[i, 46:56]*np.sqrt(td*ntr)/data[i,56:66]
    eta_cob_nom_10first_6_cameras[i, :] = Lambda_nom * data[i, 76:86]*np.sqrt(td*ntr)/data[i, 86:96]
    Lambda_ext = dback*1e-6/(1. - dback*1e-6*data_ext[i, 14:24])
    eta_cob_ext_10first_24_cameras[i, :] = Lambda_ext * data_ext[i,45:55]*np.sqrt(td*ntr)/data_ext[i,55:65]
    eta_cob_ext_10first_6_cameras[i, :] = Lambda_ext * data_ext[i,75:85]*np.sqrt(td*ntr)/data_ext[i,85:95]
    ...
    #Lambda_sec = dback*1e-6/(1. - dback*1e-6*data_sec[i, 5])
    #eta_cob_sec_24_cameras[i, :] = Lambda_sec * data_sec[i, 9] / data_sec[i, 10]
    #eta_cob_sec_6_cameras[i, :] = Lambda_sec * data_sec[i, 12] / data_sec[i, 14]
...
eta_cob = data[:, 14] / data[:,16]
eta_cob_6_cameras = data[:, 42] / data[:, 43]
