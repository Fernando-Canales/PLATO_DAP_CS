...
n_tar = 2                            # number of targets per magnitude interval
...
save_info = np.zeros((n_tar * nP, 17))     # numpy arr. to store the metrics for the nominal mask
save_info_sec = np.zeros((n_tar * nP, 11)) # numpy arr. to store the metrics for the secondary mask
save_info_ext = np.zeros((n_tar * nP, 33)) # numpy arr. to store the metrics for the extended mask
...
for i in range(nP):
    ...
    def process_target(k): # main processing routine (works on a given TARGET)
        ...
        eta_cob, sigma_1_24, abs_cob = centroid_shift(w=w_t, Ik=Ic_max, I_t=It, I_contaminants=Ic_acc, 
                                            sprk=sprk[ind_sprk], dback=dback, sb=sb, sd=sd, sq=sq, td=td, ntr=ntr)
        # -------------------------------------------NOMINAL COB-------------------------------------------------------#

        # ------------------------------------------SECONDARY COB------------------------------------------------------#
        eta_cob_c, sigma_1_24_c, abs_cob_c = centroid_shift(w=w_c, Ik=Ic_max, I_t=It, I_contaminants=Ic_acc, 
                                            sprk=sprk_sec[ind_sprk], dback=dback, sb=sb, sd=sd, sq=sq, td=td, ntr=ntr)
        # ------------------------------------------SECONDARY COB------------------------------------------------------#
        ################################################################################################################
        #                                               EXTENDED MASK                                                  #
        ################################################################################################################
        ...
        eta_ext = sprk_ext[ind_sprk] * np.sqrt(td * ntr) * dback / (NSR_ext_1h *(1 - SPR_tot_ext)) # w.r.t the nominal mask
       

        # ------------------------------------------EXTENDED COB-------------------------------------------------------#
        eta_cob_ext, sigma_1_24_ext, abs_cob_ext = centroid_shift(w=w_ext, Ik=Ic_max, 
        ...
        save_info_contaminant = np.array([ID_t, m_t, w_c_key, w_c_size, NSR1h_c, spr_tot_c, eta_c, delta_obs_c, abs_cob_c, eta_cob_c, sigma_1_24_c])
