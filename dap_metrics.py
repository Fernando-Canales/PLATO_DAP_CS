...
save_info = np.zeros((n_tar * nP, 37))     # numpy arr. to store the metrics for the nominal mask
save_info_sec = np.zeros((n_tar * nP, 11)) # numpy arr. to store the metrics for the secondary mask
save_info_ext = np.zeros((n_tar * nP, 34)) # numpy arr. to store the metrics for the extended mask
...
for i in range(nP):
    ...
    def process_target(k): # main processing routine (works on a given TARGET)
        ...
        print('SPR_10_first', SPRk_ext_10first)
        print('eta_10first', eta_10first)
        print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n')
