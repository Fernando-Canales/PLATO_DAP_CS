"""
Script for computing the efficiency in different
quadrants of the FP of PLATO
Fernando 27th Nov. 2024
"""
...
DIRout =  '/home/fercho/double-aperture-photometry/simulation_results/rings/quarters/1000_targets_per_magnitude_bin_fixed_dback_132000ppm_and_td_1_422_hr/'
...
number_of_circles = 6   # Number of circles   
...
# Lists to store results for each quadrant
quadrant_names = ['Q1', 'Q2', 'Q3', 'Q4']
quadrant_results_nominal = [[] for _ in range(4)]
quadrant_results_secondary = [[] for _ in range(4)]
quadrant_results_extended = [[] for _ in range(4)]
quadrant_results_bray = [[] for _ in range(4)]
...
# Determine quadrants based on x and y coordinates
x_coords = x_tar_fp_mm
y_coords = y_tar_fp_mm

# Define circle center and radius
circle_center_x = 0.0  # Assuming the circle is centered at (0, 0)
circle_center_y = 0.0
circle_radius = 86.0   # Replace with the actual radius of your circle in mm
max_x = 85.25278781219596
min_x = -85.25278781219596
max_y = 85.09944117925542
min_y = -85.09944117925542

# Create a mask for targets within the circle
distances_from_center = np.sqrt((x_coords - circle_center_x) ** 2 + (y_coords - circle_center_y) ** 2)
circle_mask = distances_from_center <= circle_radius
...
quadrant_masks = [
    ...
    ((x_coords >= 0) & (x_coords <= max_x) & (y_coords <= 0) & (y_coords >= min_y) & circle_mask),
]

# Process targets in each quadrant
for idx, mask in enumerate(quadrant_masks):
    quadrant_name = quadrant_names[idx]
    targets_in_quadrant = np.where(mask)[0]  # Indices of targets in the current quadrant
    n_targets_quadrant = len(targets_in_quadrant)
    ...
    for k in targets_in_quadrant:
        ...
        quadrant_results_bray[idx].append(results[3])

# Save each quadrant's results as separate .npy files
for idx, quadrant_name in enumerate(quadrant_names):
    nominal_results = np.array(quadrant_results_nominal[idx])
    secondary_results = np.array(quadrant_results_secondary[idx])
    extended_results = np.array(quadrant_results_extended[idx])
    bray_results = np.array(quadrant_results_bray[idx])
    
    np.save(f"{DIRout}quadrant_{quadrant_name}_nominal.npy", nominal_results)
    np.save(f"{DIRout}quadrant_{quadrant_name}_secondary.npy", secondary_results)
    np.save(f"{DIRout}quadrant_{quadrant_name}_extended.npy", extended_results)
    np.save(f"{DIRout}quadrant_{quadrant_name}_bray.npy", bray_results)

print("Quadrant-based processing and saving complete.")
